Page({
  onReady() {
    ...
    const targetHeights = [420, 390, 440]; // 目标高度数组
    ...
    const animate = ()=>{
      ...
      // 绘制柱子
      for (let i = 0; i < 3; i++) {
        this.drawBar(ctx, i, currentHeights[i], barWidth, barGap, canvasHeight, colors[i]);
      }
      ...
      if(finished){
        ...
        // 绘制水平箭头和文字（调整位置和样式）
        this.drawHorizontalArrow(ctx, barWidth, barGap, canvasHeight, 
        ...
        // 绘制柱子顶部数值
        this.drawValueText(ctx, 0, barWidth, barGap, '$3.1T', canvasHeight, targetHeights[0]);
        this.drawValueText(ctx, 1, barWidth, barGap, '$3.3T', canvasHeight, targetHeights[1]);
        this.drawValueText(ctx, 2, barWidth, barGap, '$3.15T', canvasHeight, targetHeights[2]);
      }
  ...
  // 绘制柱子（增强立体效果）
  drawBar(ctx, index, height, barWidth, barGap, canvasHeight, color) {
    const x = barGap + (barWidth + barGap) * index;
    ...
    if(index==1){
      const side = 15;
      const x = barGap + (barWidth + barGap) * 1
      ...
      ctx.fillText('$2.9T', x + width / 2, y + solidHeight / 2 - 150);
      ctx.restore();
      ...
      // 绘制柱子左侧面（蓝色调）
      ctx.beginPath();
      const leftSideColor = this.lightenColor(color, 15);
      ...
      // 绘制柱子右侧面（更亮的蓝色调）
      ctx.beginPath();
      const rightSideColor = this.lightenColor(color, 30);
      ...
      // 绘制柱子顶部
      ctx.beginPath();
      const topColor = this.lightenColor(color, 40);
      ctx.moveTo(x, canvasHeight - height);
    ...
    // 绘制倒影
    this.drawReflection(ctx, x, canvasHeight - height, barWidth + 30, height, index);
  },
  
  // 绘制倒影
  drawReflection(ctx, x, y, width, height,index) {
  ...
  // 绘制水平箭头和文字
  drawHorizontalArrow(ctx, barWidth, barGap, canvasHeight, text, startX, startY, arrowLength) {
    const arrowHeadSize = 7; // 箭头头部大小
    const arrowYOffset = -40; // 上移40像素，可根据需要调整
    
    // 绘制箭头线
    ctx.beginPath();
    ctx.moveTo(startX, startY + arrowYOffset);
    ctx.lineTo(startX + arrowLength, startY + arrowYOffset);
    ctx.strokeStyle = '#bfc2cb';
    ctx.lineWidth = 1;
    ctx.stroke();
    ...
    // 绘制箭头头部
    ctx.beginPath();
    ctx.moveTo(startX + arrowLength, startY + arrowYOffset);
    ctx.lineTo(startX + arrowLength - arrowHeadSize, startY + arrowYOffset - arrowHeadSize / 2);
    ctx.lineTo(startX + arrowLength - arrowHeadSize, startY + arrowYOffset + arrowHeadSize / 2);
    ctx.closePath();
    ctx.fillStyle = '#bfc2cb';
    ctx.fill();
    ...
    lines.forEach((l, i) => {
      // 文字放在箭头上方
      ctx.fillText(l, startX + arrowLength / 2, textY + i * lineHeight - 150);
    });
  
  },
  
  // 绘制柱子顶部数值
  drawValueText(ctx, index, barWidth, barGap, value, canvasHeight, barHeight) {
